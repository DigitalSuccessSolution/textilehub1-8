  ...
  return (
    <div className="pb-16 max-w-4xl mx-auto px-4">
      ...
      <div className="text-center py-10 mb-6">
        <h1 className="font-playfair text-4xl lg:text-5xl text-[#1B484E] font-bold">Notice Board</h1>
      </div>
        ...
        {notices.map((notice) => (
          <div
            ...
            className={`group bg-white rounded-2xl border px-8 py-6 cursor-pointer flex items-center justify-between hover:shadow-lg transition-all duration-300 ${
              notice.isNew ? 'border-l-8 border-[#A8C6B6]' : 'border-gray-100'
            }`}
          >
            <div className="flex items-start gap-5">
              <div className={`p-3 rounded-full shrink-0 ${notice.isNew ? 'bg-[#A8C6B6]/10' : 'bg-gray-50'}`}>
                <Bell size={20} className={notice.isNew ? 'text-[#A8C6B6]' : 'text-gray-400'} />
              </div>
              <div>
                <div className="flex items-center gap-3 mb-2">
                  <span className="text-xs text-gray-500 font-bold uppercase tracking-wider">{notice.date}</span>
                  {notice.isNew && (
                    <span className="bg-[#A8C6B6] text-white text-[10px] px-2.5 py-0.5 rounded-full uppercase tracking-widest font-bold shadow-sm">NEW</span>
                  )}
                </div>
                <h3 className="text-lg font-bold text-[#1B484E] group-hover:text-[#A8C6B6] transition-colors">{notice.title}</h3>
              </div>
            </div>
